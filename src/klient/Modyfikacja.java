...
public class Modyfikacja extends JFrame implements ActionListener {
	...
	JButton btnZarejestruj = new JButton("Modyfikuj");
	...
	public Modyfikacja() {
		...
		this.setTitle("Modyfikacja danych");
		this.setVisible(true);
		
		b.wczytklienci("SELECT * FROM uzytkownicy WHERE Adres_email='"+Login.email+"'");
		
		Klienci k=Klienci.listaklient.get(0);
		...
		textField_1.setText(k.getNazwisko());
		...
		passwordField_1.setText(k.getHaslo());
		...
		textField_2.setText(k.getEmail());
		...
		textField_3.setText(k.getNrtelefonu());
		
		passwordField = new JPasswordField();
		passwordField.setBounds(146, 118, 86, 20);
		contentPane.add(passwordField);
		passwordField.setText(k.getHaslo());
		...
		textField_4.setText(k.getMiejscowosc());
		...
		textField_6.setText(k.getUlica());
		...
		textField_7.setText(k.getNrdomu());
		...
		textField_8.setText(k.getNrlokalu());
		
		textField_9 = new JTextField();
		textField_9.setColumns(10);
		textField_9.setBounds(426, 203, 40, 20);
		contentPane.add(textField_9);
		textField_9.setText(cz[1]);
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if(e.getSource()==btnZarejestruj)
		{
			...
			if(imie.length()!=0 && nazwisko.length()!=0 && haslo.length()!=0 && haslo2.length()!=0 && miejscowosc.length()!=0 && kod.length()!=0 && ulica.length()!=0 && nrdomu.length()!=0 && nrtelefonu.length()!=0 && email.length()!=0)
			{
				if(haslo.equals(haslo2))
				{
					if(textField_5.getText().length()==2 && textField_5.getText().matches("^[0-9]+$") && textField_9.getText().length()==3 && textField_9.getText().matches("^[0-9]+$"))
					{
						if(nrtelefonu.matches("^[0-9]+$"))
						{
							b.sprid("SELECT ID_Uzytkownika FROM uzytkownicy WHERE Adres_email='"+Login.email+"'", 1);
							b.modyfikacjaklient(imie, nazwisko, haslo, miejscowosc, kod, ulica, nrdomu, nrlokalu, nrtelefonu, email, b.trzeci);
							JOptionPane.showMessageDialog(null, "Dane zmodyfikowano pomyślnie");
							OknoPowitalne op=new OknoPowitalne();
							op.setVisible(true);
							this.setVisible(false);
						}
